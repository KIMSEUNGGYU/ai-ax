# ai-ax

Claude Code 플러그인 모음. FE 개발 워크플로우 자동화 + 세션 관리.

## 플러그인

| 플러그인 | 버전 | 역할 |
|----------|------|------|
| [fe-workflow](./fe-workflow/) | v0.4.0 | FE 컨벤션 기반 설계 → 리뷰 → PR |
| [learning](./learning/) | v0.1.0 | 개인 세션 관리 + 학습 추출 + 문서 정리 |
| [session-manager](./session-manager/) | v0.1.0 | 세션 상태 저장, 습관 측정 |
| session-wrap (외부) | - | 세션 종료 분석/제안 ([설치](https://github.com/team-attention/plugins-for-claude-natives)) |

## 설치

```bash
# 개발 중 (플러그인 동시 로드)
claude --plugin-dir ./fe-workflow --plugin-dir ./learning --plugin-dir ./session-manager

# 마켓플레이스 등록 후
/plugin marketplace add /path/to/ai-ax
/plugin install fe-workflow@claude-plugins
/plugin install session-manager@claude-plugins
```

## 워크플로우

```
세션 시작 (자동: STATUS.md 로드 + 세션 ID 생성)  ← session-manager
    ↓
/architecture 기능명                              ← fe-workflow
    ↓
대화로 구현                                       ← 기본
    ↓
/review                                          ← fe-workflow
    ↓
/pr                                              ← fe-workflow
    ↓
/note 주제명                                     ← session-manager (개발 중 아무 때나)
    ↓
/wrap                                            ← session-wrap (세션 분석/제안)
    ↓
/save                                            ← session-manager (상태 저장 + 점수)
```

- `/dashboard`는 아무 때나 실행 가능 (습관 대시보드)

## 커맨드 요약

### learning

| 커맨드 | 역할 |
|--------|------|
| `/learning:clear` | 세션 종료 — 컨텍스트 저장 + 패턴 승격 |
| `/learning:resume` | 이전 세션 컨텍스트 복원 |
| `/learning:doc` | 내용 정리 → 프로젝트/Obsidian 저장 |

### fe-workflow

| 커맨드 | 역할 |
|--------|------|
| `/architecture` | 컨벤션 기반 설계 → 구현 지시서 생성 |
| `/review` | 컨벤션 기반 코드 리뷰 → 점수/피드백 |
| `/pr` | 변경사항 분석 → PR 생성 |

### session-manager

| 커맨드 | 역할 |
|--------|------|
| `/save` | 세션 상태 저장 — STATUS.md + 로그 + 습관 점수 |
| `/dashboard` | 습관 대시보드 — 점수 추이, 개선 포인트 |
| `/note` | 개발 중 패턴/개념 정리 → md 저장 |

### session-wrap (외부)

| 커맨드 | 역할 |
|--------|------|
| `/wrap` | 세션 분석 — 문서 업데이트, 자동화 기회, 학습 추출, 후속 작업 |
